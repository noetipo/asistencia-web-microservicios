  ...
  template: `
      ...
      <div class="responsive-table">
          <table class="table table-lg table-hover table-striped table-sm">
              ...
              <tbody class="table-group-divider">
              <tr *ngFor="let v of tuitions ; let i=index">
                  <th scope="row">{{i + 1}}</th>
                  <td data-title="Código">{{v.personaDto?.codigo}}</td>
                  <td data-title="DNI">{{v.personaDto?.dni}}</td>
                  <td data-title="Nombres">{{v.personaDto?.nombres}}</td>
                  <td data-title="Ciclo">{{v.personaDto?.ciclo}}</td>
                  <td data-title="Grupo">{{v.personaDto?.grupo}}</td>
                  <td data-title="Institucion">{{v.personaDto?.escuelaProfesional!.facultad!.filial!.institucion!.descripcion!}}</td>
                  <td data-title="Filial">{{v.personaDto?.escuelaProfesional!.facultad!.filial!.descripcion!}}</td>
                  <td data-title="Facultad">{{v.personaDto?.escuelaProfesional!.facultad!.descripcion!}}</td>
                  <td data-title="E.P.">{{v.personaDto?.escuelaProfesional!.descripcion!}}</td>
                  <td data-title="Estado">
                        <span class="badge text-bg-{{v.personaDto?.estado?'success': 'danger'}} text-white">
                            {{v.personaDto?.estado ? 'Activo' : 'Inactivo'}}
                        </span>
                  </td>
...
export class EventEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(res => {
      this.idEvent = parseInt(res['idEvent']);
      this.eventGetById(this.idEvent);
    });
  ...
  public eventGetById(idEvent: number): void {
    const listById = this.eventService.getById$(idEvent).subscribe(async response => {
      this.event = response || {};
      this.eventPathValue(this.event);
  ...
  public eventPathValue(event: Event): void {
    ...
    this.eventForm.patchValue({
      institucionId: event.escuelaProfesionalDto?.facultad?.filial?.institucion?.id,
      filialId: event.escuelaProfesionalDto?.facultad?.filial?.id,
      facultadId: event.escuelaProfesionalDto?.facultad?.id,
      escuelaProfesionalId: event.escuelaProfesionalDto?.id,
    });
    event.eventoDetalles!.map(data => {
      data.fecha = this.patchDateObject(data.fecha);
  ...
  public saveForm(): void {
    ...
    this.tuitions.map(data => {
        const dataPerson: any = {};
        dataPerson.id=data?.id!;
        dataPerson.personaId =data.personaDto?.id;
  ...
  private patchDateObject(date: any) {
    if (date && date.year && date.month) {
      ...
      if (date && date.split('-')) {
        const s = date.split('-');
        return {year: +s[0], month: +s[1], day: +s[2].substr(0, 2)};
      } else {
        return null;
      }
    }
