...
export class PersonEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(res => {
      this.idPerson = parseInt(res['idPerson']);
      this.personGetById(this.idPerson);
    });
  ...
  public personGetById(idPerson: number): void {
    const listById = this.personService.getById$(idPerson).subscribe(async response => {
      this.person = response || {};
      this.personPathValue(this.person);
      listById.unsubscribe();
    });
  }

  public personPathValue(person: Person): void {
    // @ts-ignore
    this.personForm.patchValue(person);
    this.personForm.patchValue({
      institucionId: person.escuelaProfesional?.facultad?.filial?.institucion?.id,
      filialId: person.escuelaProfesional?.facultad?.filial?.id,
      facultadId: person.escuelaProfesional?.facultad?.id,
      escuelaProfesionalId: person.escuelaProfesional?.id,
      id: this.idPerson,
    });
  }
  ...
  public saveForm(): void {
    if (this.personForm.valid) {
      ...
      this.confirmDialogService.confirmSave().then(() => {
        this.personService.updateObject$(this.personForm.value).subscribe(response => {
